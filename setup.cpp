...
void createObjects(plist &people, olist &objects){
	...
	for (int i = 0; i < people.size(); ++i)
	{
		if(people[i][3] != " "){
			...
			{
				if(objects[j]->name == people[i][3] && (abs(objects[j]->grade - objects[i]->grade) <= 1))
		...
		if(people[i][4] != " "){
			for (int j = 0; j < objects.size(); ++j)
			{
				if(objects[j]->name == people[i][4] && (abs(objects[j]->grade - objects[i]->grade) <= 1))
		...
		if(people[i][5] != " "){
			for (int j = 0; j < objects.size(); ++j)
			{
				if(objects[j]->name == people[i][5] && (abs(objects[j]->grade - objects[i]->grade) <= 1))
